...
export function checkExistKey(config: ProviderSettings | undefined) {
	...
	// Special case for human-relay, fake-ai, claude-code, qwen-code, oca and roo providers which don't need any configuration.
	if (
		config.apiProvider &&
		["human-relay", "fake-ai", "claude-code", "qwen-code", "roo", "gemini-cli", "oca"].includes(config.apiProvider) // kilocode_change: add gemini-cli
	) {
		return true
	}
